-- ============================================
-- MIGRATION: RPC transacional transferir_devolucoes_estoque
-- ============================================
-- 1 RPC SECURITY DEFINER que executa toda a transferência atomicamente
-- Idempotência via request_id
-- Validações: destino_id, devolucao_id duplicado, valor_restante
-- FOR UPDATE ORDER BY devolucao_id para evitar deadlock
-- ============================================
...
BEGIN
  ...
  IF FOUND THEN
    RETURN v_existing;  -- Retornar mesmo resultado de sucesso já persistido
  END IF;
  ...
  PERFORM 1 FROM devolucoes_estoque
  ...
  LOOP
    ...
    IF NOT EXISTS (SELECT 1 FROM devolucoes_estoque WHERE id = v_devolucao_rec.devolucao_id AND empresa_id = v_empresa_id) THEN
  ...
  INSERT INTO operacoes_estoque (empresa_id, estoque_id, tipo_operacao, data, face_titulos, valor_compra, despesas, recompra, liquido_operacao, historico, created_by)
  VALUES (
    v_empresa_id,
    v_estoque_devolucoes_id,
    'saida',
    v_data_transferencia,
    0, 0, 0, 0, v_valor_total,
  ...
  -- 11. Criar movimentação
  INSERT INTO movimentacoes_estoque (operacao_estoque_id, tipo, valor, data, historico, estoque_origem_id, conta_bancaria_id, estoque_destino_id)
  SELECT
    v_operacao_saida_id,
  ...
  -- 13. Processar destino
  IF v_destino_tipo = 'conta' THEN
    PERFORM public.increment('contas_bancarias', 'id', v_destino_id::UUID, 'saldo_atual', v_valor_total);
    INSERT INTO lancamentos_caixa (empresa_id, conta_bancaria_id, data, historico, tipo, valor, observacoes)
    VALUES (
      v_empresa_id,
      v_destino_id::UUID,
      v_data_transferencia,
  ...
  ELSE
    ...
    INSERT INTO operacoes_estoque (empresa_id, estoque_id, tipo_operacao, data, face_titulos, valor_compra, despesas, recompra, liquido_operacao, historico, observacoes, created_by)
    VALUES (
      v_empresa_id,
...
COMMENT ON FUNCTION public.transferir_devolucoes_estoque IS 'RPC transacional: transferência de devoluções com idempotência (request_id)';
