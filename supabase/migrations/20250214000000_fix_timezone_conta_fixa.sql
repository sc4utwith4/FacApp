-- ============================================
-- Correção de Timezone para Contas Fixas
-- ============================================
-- Esta migration cria uma função RPC para inserir contas fixas
-- garantindo que a data seja tratada como local sem conversão de timezone

-- Função para criar conta fixa com tratamento correto de data
...
DECLARE
    ...
    v_proximo_evento_text TEXT;
    v_result RECORD;
BEGIN
    -- Garantir que recebemos string pura (pode vir como TEXT, VARCHAR, ou até DATE do Supabase)
    -- Converter explicitamente para TEXT para evitar problemas de tipo
    v_proximo_evento_text := COALESCE(p_proximo_evento::TEXT, '');
    
    -- Validar formato de data (YYYY-MM-DD)
    IF v_proximo_evento_text !~ '^\d{4}-\d{2}-\d{2}$' THEN
        RAISE EXCEPTION 'Formato de data inválido. Esperado YYYY-MM-DD, recebido: %', v_proximo_evento_text;
    END IF;
    
    -- Converter string de data para DATE usando TO_DATE para garantir tratamento local
    -- TO_DATE trata a string como data local sem conversão de timezone
    -- IMPORTANTE: TO_DATE não considera timezone, trata como data literal
    v_proximo_evento_date := TO_DATE(v_proximo_evento_text, 'YYYY-MM-DD');
...
-- Função para atualizar conta fixa com tratamento correto de data
