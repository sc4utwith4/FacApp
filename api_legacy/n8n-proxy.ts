 ...
 * IMPORTANTE: Configurado para plano Free do Vercel (limite de 10 segundos).
 * - Free/Hobby: 10s (10000ms) - LIMITE HARD
 * - Pro: 60s (60000ms) - requer maxDuration no vercel.json
 * - Enterprise: 900s (900000ms)
 * 
 * Timeout pode ser configurado via variável de ambiente N8N_PROXY_TIMEOUT_MS.
 * Default: 7000ms (7s) para Vercel Free, deixando 3s de margem para overhead.
 * 
...
function determineTimeout(question: string): number {
  // Ler timeout da variável de ambiente, com fallback para 7s (compatível com Vercel Free)
  const envTimeout = Number(process.env.N8N_PROXY_TIMEOUT_MS);
  const MAX_TIMEOUT = Number.isFinite(envTimeout) && envTimeout > 0 ? envTimeout : 7000; // 7 segundos default
  
  // No plano Free, todas as perguntas usam o mesmo timeout
  // Se precisar de mais tempo, configure N8N_PROXY_TIMEOUT_MS ou faça upgrade para Pro
  return MAX_TIMEOUT;
}
