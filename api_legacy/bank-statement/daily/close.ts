import type { DailyCloseRequest } from '../../../src/types/bank-reconciliation.js';
import {
  callUserRpc,
  extractBearerToken,
  getAdminClient,
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  let body: DailyCloseRequest;
  try {
    body = (parseJsonBody(req) || {}) as DailyCloseRequest;
  } catch {
  ...
  if (serviceRoleKey) {
    ...
    await safeInsertBankAuditLog(adminClient, {
      ...
      status: 'success',
      message: 'Fechamento diario executado com sucesso.',
