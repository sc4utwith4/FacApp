...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const contaBancariaId = readQueryValue(req.query?.conta_bancaria_id) || null;
  const dataReferencia = readQueryValue(req.query?.data_referencia) || null;
  ...
  const buildSessionQuery = (withArchivedFilter: boolean) => {
    ...
    } else if (contaBancariaId && dataReferencia) {
      query = query
  ...
  if (!sessionId && !(contaBancariaId && dataReferencia)) {
    return res.status(400).json({
      error: 'Invalid input',
      message: 'Informe session_id ou (conta_bancaria_id + data_referencia).',
    });
  }
