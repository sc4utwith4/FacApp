...
/**
 * set-vercel-operacoes-ia-env.cjs
 *
 * Configura as variáveis de ambiente da Vercel necessárias para a integração
 * do Assistente de Operações com IA (webhook n8n).
 *
 * Variáveis de ambiente esperadas:
 *   N8N_OPERACOES_IA_WEBHOOK_URL           (obrigatório)
 *   N8N_OPERACOES_IA_INTEGRATION_SECRET    (obrigatório)
 *   VERCEL_TOKEN                           (ou usar auth local da CLI)
 *   VERCEL_TARGETS                         (opcional, default: production,preview)
 *
 * Uso:
 *   N8N_OPERACOES_IA_WEBHOOK_URL="https://workflow.epistemecompany.com.br/webhook/operacoes-ia/chat" \
 *   N8N_OPERACOES_IA_INTEGRATION_SECRET="sua-chave-secreta" \
 *   node scripts/set-vercel-operacoes-ia-env.cjs
 */
...
function parseTargets(raw) {
  ...
  const out = value.split(',').map((t) => t.trim().toLowerCase()).filter(Boolean);
...
async function main() {
  ...
  const webhookUrl = String(process.env.N8N_OPERACOES_IA_WEBHOOK_URL || '').trim();
  const integrationSecret = String(process.env.N8N_OPERACOES_IA_INTEGRATION_SECRET || '').trim();
  const targets = parseTargets(process.env.VERCEL_TARGETS);

  if (!webhookUrl) {
    throw new Error('N8N_OPERACOES_IA_WEBHOOK_URL obrigatório.');
  }
  if (!integrationSecret) {
    throw new Error('N8N_OPERACOES_IA_INTEGRATION_SECRET obrigatório.');
  }
  ...
  const desired = [
    { key: 'N8N_OPERACOES_IA_WEBHOOK_URL', value: webhookUrl },
    { key: 'N8N_OPERACOES_IA_INTEGRATION_SECRET', value: integrationSecret },
  ];
  ...
  for (const item of desired) {
    ...
    for (const match of matches) {
      await apiRequest(token, 'DELETE', `${base}/env/${encodeURIComponent(match.id)}?${teamParam}`, null);
    }
  ...
  console.log(`\nVercel envs Operações IA atualizadas no projeto ${projectName || projectId}.`);
